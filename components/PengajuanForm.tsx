    ...
    (async () => {
      ...
      if (mode === "edit" && pengajuanId) {
        ...
        if (existing) {
          ...
          if (potonganData) {
            // Gabungkan potongan tersimpan ke template default supaya semua
            // jenis pajak standar tetap kelihatan meski nominalnya 0.
            setPotongan((prev) =>
              prev.map((p) => {
                const found = (potonganData as Potongan[]).find((x) => x.jenis_pajak === p.jenis_pajak);
                return found ? { ...p, nominal: found.nominal } : p;
              })
            );
          }
        }
